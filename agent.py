...
# define agent number of moves
agent_moves = 50

# define probability of each color
...
def agent_move(next_ball_color, next_ball_radius):
    x = random.randint(40, 560)  # Random x-coordinate within the allowed range
    y = 40  # y-coordinate is fixed at the top of the box
    print(f"Agent is dropping a ball at ({x}, {y})")  # Debugging print
    # add delay to agent move
    time.sleep(1)
    horizontal_speed = 0 
    is_dropping = True
    balls.append([x, y, ball_speed, horizontal_speed, next_ball_color, next_ball_radius, is_dropping])

    # Update the next ball's color and radius for the next move
    next_index = random.choices(index, weights=skwed_probabitity, k=1)[0]
    return color_order[next_index], radius_order[next_index]



clock = pygame.time.Clock()  # Add at the start of your game


while running:
    # clock.tick(120)  # Limit the game to 60 frames per second
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if 40 < event.pos[0] < 560 and event.pos[1] < 40:                # draw a ball when mouse is clicked
                ball_x = event.pos[0]
                ball_y = event.pos[1]
                horizontal_speed = 0 
                is_dropping = True
    ...
    # agent move
    # Agent's move
    if not game_over and agent_moves > 0:
        next_ball_color, next_ball_radius = agent_move(next_ball_color, next_ball_radius)
        agent_moves -= 1
        print(f"agent moves left: {agent_moves}")
    ...
    if not game_over:
        ...
        # Debug: Print the number of balls and their positions
        ...
        for ball in balls:
            ...
            if (not ball[6]) and ball[1] - ball[5] < box_top:
                print(f"Game over triggered by ball at position: {ball[0]}, {ball[1]}")
                game_over = True
                break
            # ball[2] += gravity
            ball[2] += gravity  # Increase the ball's vertical speed
            ball[1] += ball[2]  # Update the ball's vertical position
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_SPACE:
                    ...
                    running = False
    pygame.display.update()  # Make sure this is called at the end of your loop
